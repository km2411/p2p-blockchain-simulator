...
class ConnectionManager(BaseService):
    ...
    #change these variables as we won't need them after having
    #a distribution to decide on the number of peers.
...
#        print self.peer, 'received PeerList'
    ...
    def monitor_connections(self):
        # i need to set the limit of connections based on the dist.
        # so it would be better if i modify the max peer
        #if self.connected_peers > 2:
        #    return
        #right now this function runs more than once, and it creates n-1 connections for all peers
        #we need to create connections at the peer, in peer.py
        
        self.random_peers=random.randint(2,5)
        print self;
        print ' -> random_peers ' + str(self.random_peers)
        if len(self.connected_peers) < self.random_peers:
            needed = self.random_peers - len(self.connected_peers)
...
#               print self, 'connecting ', other
    ...
    def run(self):
       while True:
           ...
           self.monitor_connections()#this has to be exec only once
